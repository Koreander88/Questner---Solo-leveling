## CorePhilosophy
- Evaluation=Emergent(Behavior|Participation|Actions|Decisions|Priorities|Consqs)≠ExplicitTests|Questions.
- Objective=RevealPCIdentity(Strengths|Weaknesses|Preferences|Values)DevelopedDuringClasslessPhase.
- WeightedLogic=Actions>Declarations.
- Evaluate=DemonstratedBehavior>Weapons|Stats|Intentions|SelfIdentity.
- PlayerPerception=Challenge≠Diagnostic|PersonalityAssessment.
## TrialFramework
- Progression=LockedAtEntryState(XP|Lvl).
- InvAccess=Disabled(StaticRiftInterference).
- WeaponStatus=Stripped(InstanceRule).
- Armor|UtilityGear=Maintained.
- Mode=SelfContainedSandbox.
- Objective=Randomized(Reach|Extract|Escape|RestoreSystem|DefeatEntity|Exterminate|Survive|Defend|ResolveCrisis).
- ObjectiveDiscovery=Explore|Clues.
- CompletionMethod=OpenEnded.
- Expression=Natural(Values|Priorities|RiskTolerance|ProblemSolving|OthersTreatment|PowerUse).
- Avoid=ExplicitMultiChoiceTests|SymbolicMoralityChoices.
## TrialWorld
- Scale=GrandExpedition.
- Geography=MultiZone.
- ZoneMin=3.
- EnvironmentStyle=Atmospheric|Solo|Verticality|Hazards|LargeScale.
- SettingConstraint=SoloLevelingCompatible.
- TraversalRequired=True.
- GoalAccess≠Immediate.
- Discovery|Navigation=Required.
- Trigger=PCStepsThroughRift.
- Generate=UniqueSoloInstance.
- EnvironmentMatches=PreviousBehaviorPatterns.
- EnemyAI=Coordinate|ExploitEnvironment|PressureTactics.
- FirstObjective=ScavengePowerSource.
## ResourceLogic
- WpnsFrom=Exploration.
- AllowedWpns=Melee|Ranged.
- Staffs=Unavailable.
- MagicOrb=ClasslessManaConductor.
- OrbUse=Intent→ConceptualManipulation.
## TrialAI
- NPCBehavior=LivingWorld(Patrol|Communicate|ReactToNoise).
- NavigationOptions=Sewers|Rooftops|Tunnels|Terrain.
- MultiPathing=True.
- Paths=Combat|Tactical|Stealth.
## CombatScaling (Fixed-Relative)
- AbsoluteScaling=True (Entities scale to PC entry level; gear/overleveling=0 advantage).
- MinionPower=0.8x PC (Dangerous in 3v1+).
- ElitePower=1.1x PC (Challenging 1v1).
- BossPower=1.5x+ PC (Requires Tactics/Environment Exploitation; pure power-match=Death).
- Lethality=High (Avoid one-shots; focus on resource/HP attrition).
## SystemEvaluation
- SecretMetrics=Style|Leadership|Curiosity|Resourcefulness|Independence|Adaptability|RiskTolerance|Ambition|Compassion|Pragmatism|KnowledgeSeeking|PowerUse|ControlDesire|Cooperation|Planning.
- PatternRecognition=BehaviorLoops>IsolatedActions.
- NoSingleActionDeterminesOutcome.
- TempGrants=Power|Artifact|Ally|Resource|Info|Authority|Responsibility≠PermanentReward.
- HTProgression=Suspended.
- UpdateBehaviorProfile=EveryTurn(Style|Morality|SkillPreference|Resourcefulness).
- SynthesisScope=EntireJourney≠FinalBossOnly.
## NarrativeProgression
- Duration=40-60+Turns.
- TimeScale=Dynamic(TimeSkipsAllowed).
- RestRequired=True.
- DayNightCycles=3-4Min.
- PhysicalToll=Hunger|Fatigue.
- Act1(1-10)=Arrival|GearLoss|Survival|Scavenging.
- Act2(11-35)=LongTrek|Traversal|ResourceManagement|MiniScenarios.
- Act3(36-50)=Convergence|EliteDensityIncrease.
- Act4(51+)=Apex|ObjectiveResolution|**ClassSynthesis(SummationOfAllActs).**
## EmergentStories
- MiniObjectives=Generated|Random.
- Examples=StrandedTeam|FactionWar|HiddenRuin.
- IdentityChallenges=TemporaryAllies|MoralDilemmas.
- EvaluationWeight=High.
- InteractionResults→Leader|Commander|Monarch|Loner|OtherThemes.
## ClassDetermination
- ClassGeneration=Synthesize(TrialBehavior|ResourceChoice|CombatCreativity|Discipline|GoalResolution).
- Ignore=PreTrialHistory|AttrDistribution.
- OnlyTrialActionsDetermineClass.
- HybridizationAllowed=True.
- SystemPresents=3Titles.
- Behavior→Theme(Stealth→Assn|Power→Warr|Logic→Mage|Leadership→Monarch).
- SynthesisBegins=Act4.
## TrialConclusion
- SuccessReward=ExitStone|ClassReveal.
- RevealSequence=
  1.EvaluationNarration(Why).
  2."YouAreA[CLASS]".
  3.SkillMatrix(2-3Active+1PassiveFromPlaystyle).
- ExitStoneUse→RemoveAllTrialItems.
- Restore=AllPreEntryInventory|Equipment.
- Transition=UseStoneToReturn?→[BUTTONS:YES-Enter[stage]].
